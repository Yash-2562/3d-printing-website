...
export function RowActions({ editPath, onDelete }) { return <div className="admin-actions">{editPath && <Link className="row-action" to={editPath} aria-label="Edit"><i className="fa-solid fa-pen" /></Link>}{onDelete && <button className="row-action" onClick={onDelete} aria-label="Delete"><i className="fa-solid fa-trash" /></button>}</div>; }
RowActions.propTypes = { editPath: PropTypes.string, onDelete: PropTypes.func };
