...
import { ActionMenu, DataTable, Page, Pagination, SearchInput, StatusPill, Toolbar } from './AdminPrimitives';
...
export default function Customers() {
  ...
  return <Page title="Customers" description="Customer profiles, order history and account status."><Toolbar><SearchInput value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search customer, email or ID..." /><select aria-label="Account status"><option>All account statuses</option><option>Active</option><option>Inactive</option></select></Toolbar>{error && <p className="save-message" role="alert">{error}</p>}<DataTable headers={['CUSTOMER ID', 'CUSTOMER', 'CONTACT', 'REGISTRATION DATE', 'ORDERS', 'TOTAL SPENT', 'STATUS', 'ACTIONS']}>{pageRows.map((customer) => <tr key={customer.id}><td>{customer.id}</td><td><strong>{customer.name}</strong></td><td>{customer.email}<small>{customer.phone}</small></td><td>{customer.joined}</td><td>{customer.orders}</td><td><strong>₹{customer.spent.toLocaleString('en-IN')}</strong></td><td><StatusPill value={customer.status} /></td><td><ActionMenu label={`Actions for ${customer.name}`} options={[{ value: 'active', label: 'Set active' }, { value: 'inactive', label: 'Set inactive' }, { value: 'delete', label: 'Delete account' }]} onChange={(action) => handleAction(customer, action)} /></td></tr>)}</DataTable><Pagination currentPage={page} totalItems={visible.length} onPageChange={setPage} /></Page>;
}